...
func (s *XiaohongshuService) GetLoginQrcode(ctx context.Context) (*LoginQrcodeResponse, error) {
	...
	if !loggedIn {
		logrus.Infof("xhs login qrcode issued; waiting up to %s for scan confirmation", timeout)
		go func() {
			...
			if loginAction.WaitForLogin(ctxTimeout) {
				if er := saveCookies(page); er != nil {
					...
					logrus.Infof("xhs login cookies saved successfully")
				}
			} else {
				logrus.Warnf("xhs login qrcode expired without confirmed login")
			}
		}()
	}
...
func saveCookies(page *rod.Page) error {
	...
	logrus.Infof("saved %d browser cookies to %s", len(cks), cookiePath)
	return nil
}
